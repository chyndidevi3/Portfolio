...
CREATE TEMPORARY TABLE port_osteo.tbl_id SELECT Id FROM port_osteo.osteoporosis GROUP BY Id HAVING Count(Id)>1;
SELECT *
FROM port_osteo.tbl_id



#Create temporary table to check are the duplicate Id has the same values
CREATE TEMPORARY TABLE port_osteo.check_dupl(Id int, Age int, Gender text, Hormonal_Changes text, 
Family_History text, Race_or_Ethnicity text, Body_Weight text, Calcium_Intake text, 
Vitamin_D_Intake text, Physical_Activity text, Smoking text, Alcohol_Consumption text, 
Medical_Conditions text, Medications text, Prior_Fractures text, Osteoporosis int)

INSERT INTO port_osteo.check_dupl
SELECT Id, Age, Gender, Hormonal_Changes, Family_History, Race_or_Ethnicity, Body_Weight, Calcium_Intake, Vitamin_D_Intake, Physical_Activity, Smoking, Alcohol_Consumption, Medical_Conditions, Medications, Prior_Fractures, Osteoporosis
FROM port_osteo.tbl_id aba
INNER JOIN port_osteo.osteoporosis abb
	ON aba.Id=abb.Id
ORDER BY Id
